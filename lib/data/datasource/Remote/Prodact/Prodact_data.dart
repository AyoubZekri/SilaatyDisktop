...
class ProdactData {
  ...
  Future<List<Map<String, Object?>>> search(Map data) async {
    ...
    if (categorisuuId != null && categorisuuId.toString().isNotEmpty) {
      return await sqldb.readData(
        "SELECT * FROM products WHERE user_id = ? AND (product_name LIKE ? OR codepar LIKE ?) AND categoris_uuid = ? AND is_delete=0",
        [id, '%$query%', '%$query%', categorisuuId],
      );
    } else {
      return await sqldb.readData(
        "SELECT * FROM products WHERE user_id = ? AND (product_name LIKE ? OR codepar LIKE ?) AND is_delete=0",
        [id, '%$query%', '%$query%'],
      );
    }
  }
  ...
  Future<List<Map<String, Object?>>> getProdact(
    ...
    if (categorieId == 2) {
      result = await sqldb.readData(
        "SELECT * FROM products WHERE CAST(product_quantity AS REAL) <= 0 AND user_id = ? AND is_delete=0",
        [id],
      );
    } else {
      result = await sqldb.readData(
  ...
  Future<List<Map<String, Object?>>> getProdactnotcat(Map data) async {
    final result = await sqldb.readData(
      "SELECT * FROM products WHERE user_id = ? AND is_delete=0",
      [id],
    );
    return result;
  }
  ...
  Future<List<Map<String, Object?>>> getCatProdactbytype(
    ...
    final List<Map<String, Object?>> result;
    result = await sqldb.readData(
      "SELECT * FROM products WHERE categoris_uuid = ? AND user_id = ? AND is_delete=0",
      [categorisuuId, id],
    );
