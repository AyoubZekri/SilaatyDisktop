...
class CategorisData {
  ...
  Future<Map<String, num>> getProductsCountPerCategory() async {
    try {
      if (id == null) return {};
      final result = await _db.readData('''
        SELECT categoris_uuid, COUNT(*) as items_count
        FROM products 
        WHERE is_delete = 0 AND user_id = ?
        GROUP BY categoris_uuid
      ''', [id]);
      
      Map<String, num> map = {};
      for (var row in result) {
        if (row['categoris_uuid'] != null) {
          map[row['categoris_uuid'].toString()] = row['items_count'] as num;
        }
      }
      return map;
    } catch (e) {
      print("❌ getProductsCountPerCategory error: $e");
      return {};
    }
  }
